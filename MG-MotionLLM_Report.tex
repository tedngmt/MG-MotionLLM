...
\code{updateBoneAnglesFromJoints} is why positions are streamed: Unity rebuilds each bone's rotation
from global joint positions by aiming the bone at its child joint. Over the 22-joint map it captures
the bind pose once in avatar-local space, then:
\begin{itemize}
...
\end{itemize}
Because the stream also carries each joint's global orientation (\S\ref{sec:stream}), a
rotation-driven retarget can recover the limb axial twist that this position-based version omits.
...
the Unity side by aiming each bone at its child joint.
