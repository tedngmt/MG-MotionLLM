...
motion clip(s) and render each as a 3D skeleton animation (HumanML3D/SMPL-H body-joint
topology) with the script's body-part snippets shown in sync with the motion.
...
through the VQ-VAE + motion-to-detailed-text model and visualizes the result, so it only
needs torch/transformers/matplotlib (no evaluator wrapper, spacy, or bert-score).
...
if __name__ == '__main__':
    ...
    for name, raw_motion, is_dataset_sample in samples:
        ...
        sample_dir = sample_output_dir(args.out_dir, 'm2dt', name)
